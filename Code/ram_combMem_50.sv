...
module ram_combMem_50(	// @[Decoupled.scala:273:95]
  ...
  input          W0_en,
                 W0_clk,
  input  [402:0] W0_data,
  output [402:0] R0_data
);

  reg [402:0] Memory[0:7];	// @[Decoupled.scala:273:95]
  ...
  `ifndef SYNTHESIS	// @[Decoupled.scala:273:95]
    `ifdef RANDOMIZE_MEM_INIT	// @[Decoupled.scala:273:95]
      ...
      reg [415:0] _RANDOM_MEM;	// @[Decoupled.scala:273:95]
    `endif // RANDOMIZE_MEM_INIT
    initial begin	// @[Decoupled.scala:273:95]
      ...
      `ifdef RANDOMIZE_MEM_INIT	// @[Decoupled.scala:273:95]
        for (initvar = 0; initvar < 8; initvar = initvar + 1) begin
          _RANDOM_MEM = {{`RANDOM}, {`RANDOM}, {`RANDOM}, {`RANDOM}, {`RANDOM}, {`RANDOM}, {`RANDOM}, {`RANDOM}, {`RANDOM}, {`RANDOM}, {`RANDOM}, {`RANDOM}, {`RANDOM}};
          Memory[initvar] = _RANDOM_MEM[402:0];
        end	// @[Decoupled.scala:273:95]
      `endif // RANDOMIZE_MEM_INIT
    end // initial
  `endif // not def SYNTHESIS
  assign R0_data = R0_en ? Memory[R0_addr] : 403'bx;	// @[Decoupled.scala:273:95]
endmodule
